...
#define TDIN1 HAL_GPIO_WritePin (GPIOE,TS_MOSI_Pin,GPIO_PIN_SET)
#define TDIN0 HAL_GPIO_WritePin (GPIOE,TS_MOSI_Pin,GPIO_PIN_RESET)

#define TCLK1 HAL_GPIO_WritePin (GPIOE,TS_SCK_Pin,GPIO_PIN_SET)
#define TCLK0 HAL_GPIO_WritePin (GPIOE,TS_SCK_Pin,GPIO_PIN_RESET)

#define TCS1 HAL_GPIO_WritePin (GPIOE,TS_CS_Pin,GPIO_PIN_SET)
#define TCS0 HAL_GPIO_WritePin (GPIOE,TS_CS_Pin,GPIO_PIN_RESET)

#define DOUT HAL_GPIO_ReadPin(GPIOE, TS_MISO_Pin)
...
uint16_t TP_Read_AD(uint8_t CMD)
{
	...
	for(count=0;count<16;count++)//Read 16-bit data, only the upper 12 bits are valid
	{
		...
		delayUs(20);
		TCLK1;
		if(HAL_GPIO_ReadPin (GPIOE , TS_MISO_Pin))
		{
			Num++;
		}
	}
